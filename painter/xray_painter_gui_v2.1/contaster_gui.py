...
class ContrasterGUI:
    ...
    def setup_window(self):
        ...
        cos_label = tk.Label(options_frame, text="Частота cos (> 0)")
        cos_label.pack(anchor='nw')
        self.cos_period = tk.StringVar()
        cos_entry = tk.Entry(options_frame, textvariable=self.cos_period)
        cos_entry.pack(anchor='nw')

        cos_bias_label = tk.Label(options_frame, text="Сдвиг cos (от 0 до 1)")
        cos_bias_label.pack(anchor='nw')
        self.cos_bias = tk.StringVar()
        cos_bias_entry = tk.Entry(options_frame, textvariable=self.cos_bias)
        cos_bias_entry.pack(anchor='nw')
    ...
    def process(self, *other):
        if self.target_image is not None:
            ...
            scale_len = self.contraster.color_scale_end - self.contraster.color_scale_begin
            if len(self.cos_period.get()) > 0 and float(self.cos_period.get()) > 0 and len(self.cos_bias.get()) > 0:
                end = float(self.cos_period.get()) * np.pi
                x_arr = np.linspace((0, 0, 0), (end, end, end), scale_len, True) + 2 * np.pi * float(self.cos_bias.get())
                self.sat_arr = (np.cos(x_arr) + 1) / 2
            else:
                self.sat_arr = None
            self.contraster.create_scale_wsat(self.color_scale,
                                              sat_rate=self.saturation_scale.get() / 100,
                                              sat_arr=self.sat_arr,
            ...
            '''present_img = Image.fromarray(utils.jit_cast(np.expand_dims(self.contraster.color_scale, 0),
                                                         type_max=255, dtype=np.uint8))
            resized_present_img = present_img.resize((self.scale_width, 120))
            resized_present_img.save(self.color_scale + '.png', 'PNG')'''
